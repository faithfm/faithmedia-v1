<template>
	<v-skeleton-loader :loading="isLoading" type="list-item-two-line@50">
		<v-card :class="{ playlist }">
			<v-list>
				<v-list-item-group>
					...
					>
						...
						>
							<v-list-item-title
								><span class="text--secondary">{{ index | numbers }}</span>
								{{ track.content }}
							...
							>
								...
								<span style="float: right">
									<v-chip
										v-for="tag in track.tags ? track.tags.split(' ') : []"
										:key="tag"
										x-small
										color="#dde"
										text-color="#444"
			...
			<div>
				<v-card-actions class="d-flex justify-content-center">
					<v-btn
						v-show="playlist.length < totalResultsAvailable"
						dark
						color="red darken-2"
						@click="showMoreResults"
						>Show more Results</v-btn
					>
					<v-btn
						v-show="playlist.length < totalResultsAvailable"
						dark
						...
						>Show all {{ totalResultsAvailable }} Results</v-btn
					>
					<v-btn
						v-show="showReviews"
						dark
		...
		<SongReviewSummaryDialog
			:summaryDialog.sync="summaryDialog"
			:songReviews="songReviews"
			:playlist="playlist"
		></SongReviewSummaryDialog>
	</v-skeleton-loader>
