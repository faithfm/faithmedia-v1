...
// DEPENDENCY NOTE: We are expecting window.LaravelAppGlobals.users.permissions from our Laravel app, passed in via our Blade template.
...
export function laravelUserCan(permissionToCheck) {
    ...
    return window.LaravelAppGlobals.user.permissions.some(p => p.permission===permissionToCheck);
}
...
export function laravelUserRestrictions(permissionToCheck) {
    if (!checkUserPermissionsExist())   return false;
    const perm = window.LaravelAppGlobals.user.permissions.find(p => p.permission===permissionToCheck);
...
export function checkUserPermissionsExist() {
    ...
    if (!window.LaravelAppGlobals.user)
    ...
    if (!window.LaravelAppGlobals.user.permissions)
